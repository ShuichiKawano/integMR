...
# nu: regularization parameter for covariance matrix regularization
...
### multivariate regression with regularized covariance matrix in integrative analysis
integMRR <- function(x, y, lambda, nu, rho = 1, tol= 1e-5)
{
  ...
  for(i in 1:M) loss_new <- loss_new + 
    ...
    0.5*rho*sum( ( H[[i]] - B[[i]] + U[[i]] )^2 ) +
    0.5*log(det(Sigma[[i]])) + 0.5*nu*sum(diag(solve(Sigma[[i]])))
  ...
  while( condition > tol )
  {
    ...
    for(i in 1:M) Sigma[[i]] <- t( ( y[[i]] - rep(1, nrow(y[[i]]))%*%t(alpha[[i]]) ) - x[[i]]%*%B[[i]] ) %*%
        ( ( y[[i]] - rep(1, nrow(y[[i]]))%*%t(alpha[[i]]) ) - x[[i]]%*%B[[i]] )/nrow(y[[i]]) + nu * diag(nq)
